...
const METHOD_DISPATCH: Record<string, DbMethodFn> = {
  ...
  getSemanticEdges: (db, args) => db.getSemanticEdges(args[0] as string),
  deleteEdgesForFile: (db, args) => db.deleteEdgesForFile(args[0] as string),
  deleteStructuralEdgesForFile: (db, args) => db.deleteStructuralEdgesForFile(args[0] as string),
  ...
  pruneEmptyAutoCommunities: (db) => db.pruneEmptyAutoCommunities(),
  ...
  getAllFileIds: (db) => db.getAllFileIds(),
